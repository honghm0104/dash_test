...
df_right = pd.concat([transform(df_small) for i, df_small in df_small.groupby('id')]).reset_index(drop=True)
...
for i in range(len(fig3.data)):
    trace_mode = fig3.data[i].mode
    trace_name = fig3.data[i].name
    fig3.data[i].name = f'{trace_name} (pickup/dropoff)' if 'markers' in trace_mode else f'{trace_name} (path)'
